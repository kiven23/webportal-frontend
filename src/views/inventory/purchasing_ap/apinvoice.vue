<template>
  <v-container grid-list-md text-xs-center>
    ...
    <v-card color="white lighten-2">
      ...
      <v-card-title class="text-h5 lighten-3">AP Invoice
  <v-col class="d-flex justify-end">
    <v-chip
      ...
      style="margin:5px;"
    >
      <strong>SUPPLIER/VENDOR: {{listgoodsissue.CardName}}</strong>
    </v-chip>
...
export default {
  ...
  try {
    ...
    const res = await axios.get(
      this.$URLs.backend +
        "/api/inventory/apinvoice/reports/print?DocEntry=" +
        docentry +
        "&DocNum=" +
        docnum
...
,
   ...
   async controller(data,i,docentry) {
    try {
        var baselink;
        if(i == 1){
          baselink = this.$URLs.backend+'/api/inventory/purchasing/apinvoice/getters?' + this.activeRouteBase  
        }else if(i == 2){
          baselink = this.nextt 
        }else if(i == 3){
          baselink = this.prevtt
        }else if(i == 4){
          baselink = this.$URLs.backend+'/api/inventory/purchasing/apinvoice/getters?get=' + data +'&search='+this.search;
        }else if(i == 5){
          baselink = this.$URLs.backend+'/api/inventory/purchasing/apinvoice/getters?get=' + data +'&docentry='+docentry;
        }else if(i == 6){
          baselink = this.$URLs.backend+'/api/inventory/purchasing/apinvoice/getters?get=' + data +'&id='+docentry;
        }else if(i == 7){
          baselink = this.$URLs.backend+'/api/inventory/purchasing/apinvoice/getters?get=' + data;
        } 
...
async getItemData2(docentry){
  ...
  async getCreatedAPMhistory(){
     
    this.createdAPCMhistory = await this.controller('getcreatedapcexternal', 7);
  },
  async getUDF(){
