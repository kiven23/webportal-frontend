  ...
  <div>
    <v-container grid-list-md text-xs-center>
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            <v-toolbar-title>AR-Invoice Open Balance</v-toolbar-title>
            ...
            <h6>
               ...
               BRANCH: <i style="font-size: 13px">{{ branch.U_Branch1 }} </i>
            </h6>
            <v-spacer></v-spacer>
            <v-text-field
              v-model="search"
    ...
    >
      ...
      <v-card>
        ...
        >
          ...
          <v-toolbar-title style="color: black">AR-Invoice Open Balance Print Preview</v-toolbar-title>
          <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "DOCUMENT DATE", align: "left", value: "DocDate" },
        { text: "INVOICE #", align: "left", value: "Invoice" },
        { text: "DOCUMENT #", align: "left", value: "DocumentNo" },
        { text: "CUSTOMER NAME", align: "left", value: "CustomerName" },
        { text: "OPEN BALANCE AMOUNT", align: "left", value: "OpenBalanceAmt" },
      ],
    };
  },
  computed: {
    ...mapGetters({
      query: "ar_invoice_open_balance/QUERY",
      permissions: "userPermissions/getPermission",
      branches: "ar_invoice_open_balance/getBranchSegment",
  ...
  created() {
    this.$store.dispatch("ar_invoice_open_balance/fetchBranchSegment");
  ...
  methods: {
    printpre(){
        ...
        this.iden = 0; 
        // this.printLink = 'http://192.168.1.19:7771/api/public/reports/queries/arinvoiceopenbalance?q=printing&series='+this.branch.U_Branch1+'';
        // fetch('http://192.168.1.19:7771/api/public/reports/queries/arinvoiceopenbalance?q=printing&&series='+this.branch.U_Branch1+'').then((res)=>{
        //    this.iden = 1; 
        // })
        async function getSrc(token,  series) {
              const res = await fetch('http://192.168.1.19:7771/api/public/reports/queries/arinvoiceopenbalance?q=printing&&series='+series+'', {
              method: 'GET',
              headers: {
    ...
    generate() {
      ...
      this.$store.dispatch("ar_invoice_open_balance/generatequery", data);
    },
    refreshData() {
      let data = {
        branch: this.branch,
